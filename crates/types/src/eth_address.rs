...
const MAX_L1_ADDRESS: Felt = Felt::from_hex_unchecked("0xffffffffffffffffffffffffffffffffffffffff");
...
impl TryFrom<&Felt> for EthAddress {
    ...
    fn try_from(value: &Felt) -> Result<Self, Self::Error> {
        if value <= &MAX_L1_ADDRESS {
            ...
            buffer.copy_from_slice(&value.to_be_bytes()[12..]);
...
impl From<EthAddress> for Felt {
    fn from(value: EthAddress) -> Self {
        ...
        Self::from_be_bytes_slice_unchecked(&value.inner)
    }
}
